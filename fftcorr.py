...
class Grid:
	...
	corr_grid = np.meshgrid(self.corr_cell, self.corr_cell, self.corr_cell, indexing='ij')
	self.rcorr = np.sqrt(corr_grid[0]**2+corr_grid[1]**2+corr_grid[2]**2)*self.cell_size
	del corr_grid
...
def makeYlm(ell, m, xcell, ycell, zcell):
    ...
    if (m!=0): isqpi *= np.sqrt(2.0)    # Do this up-front, so we don't forget
    tiny = 1e-20
    ngrid = len(xcell)
...
def read_patchy(cosmology, NGC):
    ...
    if (NGC=='N'): 
	ra_rotate = -142.5
	D = read_patchy_file(Mockpath+'untar/Patchy-Mocks-DR12CMASS-N-V6C-Portsmouth-mass_0001.dat', 0, cosmology)
	R = read_patchy_file(Mockpath+'Random-DR12CMASS-N-V6C-x50.dat.gz', 51*len(D['w']), cosmology)
    else:
        ra_rotate=44
...
def setup_grid(D,R,max_sep):
    ...
    R['w'] *= -np.sum(D['w'])/np.sum(R['w'])     # Set the randoms to have negative weight
    N = {}
...
def make_patchy(NGC, file_range):
    ...
    filename = 'untar/Patchy-Mocks-DR12CMASS-%s-V6C-Portsmouth-mass_0001.dat' % (NGC)
    ...
    for f in file_range:
	# Get file %f
	filename = 'untar/Patchy-Mocks-DR12CMASS-%s-V6C-Portsmouth-mass_%04d.dat' % (NGC,f)
	D = read_patchy_file(Mockpath+filename, 0, cosmology)
...
def run(cpp):
    ...
    if (cpp>1):
	hist_corrNN, hist_corr_num, rcen = correlateCPP(DDfile, dsep, file2=RRfile)
	lapsed_time('corrNN')
